...
st.pyplot(target_count.plot.pie(autopct="%1.1f%%", figsize=(6,6),ylabel="").get_figure())

#Bar — Top 20 features by missing %
st.subheader("Top 20 features by missing %")
missing_percent=df1.isnull().mean()*100
sort=missing_percent.sort_values(ascending=False).head(20)
st.bar_chart(sort)

# Histogram — AGE_YEARS
st.subheader("Histogram AGE_YEARS")
plt.figure(figsize=(6,6))
plt.hist(df1['AGE_YEARS'],bins=10,color='skyblue',edgecolor='black')
...
plt.figure(figsize=(6,6))
plt.hist(df1['AMT_INCOME_TOTAL'],bins=30,color='red',edgecolor='black')
...
plt.figure(figsize=(6,6))
plt.hist(df1['AMT_CREDIT'],bins=20,color='y',edgecolor='black')
...
plt.figure(figsize=(6,6))
plt.boxplot(df1['AMT_INCOME_TOTAL'], patch_artist=True, boxprops=dict(facecolor='violet', color='black'),
...
st.subheader("AMT_CREDIT")
plt.figure(figsize=(6,6))
plt.boxplot(df1['AMT_CREDIT'], patch_artist=True, boxprops=dict(facecolor='lightgreen'),
            whiskerprops=dict(color='green'), capprops=dict(color='green'), medianprops=dict(color='orange'))
...
plt.figure(figsize=(6,6))
sns.countplot(x=df1["CODE_GENDER"], palette='rocket') 
...
plt.figure(figsize=(6,6))
sns.countplot(x=df1["NAME_FAMILY_STATUS"],palette='magma') 
...
plt.figure(figsize=(6,6))
sns.countplot(x=df1["NAME_EDUCATION_TYPE"],palette='vlag') 
